...
| `limit_switch_pressed_state` | `VESC_CustomSensorState_t` | ≠ `VESC_CUSTOM_SENSOR_NONE` | Какое состояние кастомного статуса вески считать «концевик нажат». |
...
| `home_position_deg` | `float` | внутри зоны лимитов | Угол выходного вала в момент завершения хоуминга. |
...
телеметрию вески. Хоуминг не запускает — серва создаётся в `VESC_SERVO_STATE_DISABLED`.
